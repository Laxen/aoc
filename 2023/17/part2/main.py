...
def path(start):
    ...
    while queue:
        ...
        if pos == (data.shape[0] - 1, data.shape[1] - 1) and steps >= (4 - 1):
            return dist
        ...
        if steps < (10 - 1): # First step is 0, hence <. We need room for 1 more step to continue, hence -1.
            if in_bounds(new_pos, data):
                heappush(queue, (dist + data[new_pos], new_pos, heading, steps + 1))

        if steps >= (4 - 1): # First step is 0, hence >= and -1.
            # Left
